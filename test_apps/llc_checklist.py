...
def check_clocks(uut):
    master_site = uut.s0.sites.split(',')[0]
    clock = eval("uut.s{}.clk".format(master_site))
    clkdiv = eval("uut.s{}.clkdiv".format(master_site))
    ms_clk_freq = eval("uut.s0.SIG_CLK_S{}_FREQ".format(master_site)).split(" ")[1]
    print "Clock is set to: {:>30}".format(clock)
    print "Site {} has CLKDIV: {}".format(master_site, clkdiv)
    print "Clock mux set to: {:>5}".format(uut.s0.SYS_CLK_FPMUX.split(" ")[1])
    print "d1 clock: {} Hz".format(uut.s0.SIG_CLK_MB_FREQ.split(" ")[1])
    print "Master site clock: {} Hz".format(ms_clk_freq)
    return None
